...
import com.wang.interviewassistant.util.Dip2PxUtil;
import com.wang.interviewassistant.util.METLengthChecker;
import com.wang.interviewassistant.util.METValidator;
...
public class MaterialEditText extends AppCompatEditText {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        autoValidate = typedArray.getBoolean(R.styleable.MaterialEditText_met_autoValidate, true);
        showClearButton = typedArray.getBoolean(R.styleable.MaterialEditText_met_clearButton, false);
        mClearResId = typedArray.getResourceId(R.styleable.MaterialEditText_met_iconClear, R.mipmap.ic_clear_grey);
    ...
    private int getPixel(int dp) {
        return Dip2PxUtil.Dip2Px(getContext(), dp);
    }
...
//        int minHeight = getPaddingTop() + getPaddingBottom() + mClearTextIcon.getIntrinsicHeight();
//        int height = View.getDefaultSize(0, heightMeasureSpec);
//        super.onMeasure(widthMeasureSpec, Math.max(height, minHeight));
//
//    }
    ...
    public boolean validate(Editable s) {
        ...
        for (METValidator validator : validators) {
            ...
            isValid = isValid && validator.isValid(s, isEmpty);
            if (!isValid) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mClearTextIcon.isVisible() && x > getWidth() - getPaddingRight() - mClearTextIcon.getIntrinsicWidth() && hasFocus() && showClearButton && isEnabled()) {
    ...
    public OnClearClickListener getListener() {
        return mListener;
    }

    public void setListener(OnClearClickListener listener) {
        mListener = listener;
    }
